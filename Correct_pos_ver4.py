...
class Correction():
    def __init__(self, dist_x, dist_y):
        self.dist_x = dist_x # 12 21 34 43
        self.dist_y = dist_y # 13 24 31 42
        self.dist_diag = round(math.sqrt(pow(dist_x,2)+pow(dist_y,2)),3) # 14 23 32 41
        self.m, self.n = 1, 2
        self.ang_a, self.ang_b = math.atan2(dist_x, dist_y), math.atan2(dist_y, dist_x)
        ...
        if 2 <= n < 4:
            ...
            if f_node[0] == '11':
                if s_node[0] == '22': # First Node = Tag1, Second Node = Tag2
                    s_x, s_y = f_x + self.dist_x * math.cos(rad), f_y + self.dist_x * math.sin(rad)
                    x3, y3 = f_x + self.dist_y * math.cos((rad + 1.5 * pi)%(2 * pi)), f_y + self.dist_y * math.sin((rad + 1.5 * pi)%(2 * pi))
                    x4, y4 = s_x + self.dist_y * math.cos((rad + 1.5 * pi)%(2 * pi)), s_y + self.dist_y * math.sin((rad + 1.5 * pi)%(2 * pi))
                    ...
                    s_x, s_y = f_x + self.dist_y * math.cos(rad), f_y + self.dist_y * math.sin(rad)
                    x2, y2 = f_x + self.dist_x * math.cos((rad + 0.5 * pi)%(2 * pi)), f_y + self.dist_x * math.sin((rad + 0.5 * pi)%(2 * pi))
                    x4, y4 = s_x + self.dist_x * math.cos((rad + 0.5 * pi)%(2 * pi)), s_y + self.dist_x * math.sin((rad + 0.5 * pi)%(2 * pi))
                    ...
                    s_x, s_y = f_x + self.dist_diag * math.cos(rad), f_y + self.dist_diag * math.sin(rad)
                    x3, y3 = f_x + self.dist_y * math.cos(rad - self.ang_a), f_y + self.dist_y * math.sin(rad - self.ang_a)
                    x2, y2 = f_x + self.dist_x * math.cos(rad + self.ang_b), f_y + self.dist_x * math.sin(rad + self.ang_b)
                ...
                if s_node[0] == '11': # First Node = Tag2, Second Node = Tag1
                    s_x, s_y = f_x + self.dist_x * math.cos(rad), f_y + self.dist_x * math.sin(rad)
                    x4, y4 = f_x + self.dist_y * math.cos((rad + 0.5 * pi)%(2 * pi)), f_y + self.dist_y * math.sin((rad + 0.5 * pi)%(2 * pi))
                    x3, y3 = s_x + self.dist_y * math.cos((rad + 0.5 * pi)%(2 * pi)), s_y + self.dist_y * math.sin((rad + 0.5 * pi)%(2 * pi))
                    ...
                    s_x, s_y = f_x + self.dist_diag * math.cos(rad), f_y + self.dist_diag * math.sin(rad)
                    x4, y4 = f_x + self.dist_y * math.cos(rad + self.ang_a), f_y + self.dist_y * math.sin(rad + self.ang_a)
                    x1, y1 = f_x + self.dist_x * math.cos(rad - self.ang_b), f_y + self.dist_x * math.sin(rad - self.ang_b)
                    ...
                    s_x, s_y = f_x + self.dist_y * math.cos(rad), f_y + self.dist_y * math.sin(rad)
                    x1, y1 = f_x + self.dist_x * math.cos((rad + 1.5 * pi)%(2 * pi)), f_y + self.dist_x * math.sin((rad + 1.5 * pi)%(2 * pi))
                    x3, y3 = s_x + self.dist_x * math.cos((rad + 1.5 * pi)%(2 * pi)), s_y + self.dist_x * math.sin((rad + 1.5 * pi)%(2 * pi))
                ...
                if s_node[0] == '11': # First Node = Tag3, Second Node = Tag1
                    s_x, s_y = f_x + self.dist_y * math.cos(rad), f_y + self.dist_y * math.sin(rad)
                    x4, y4 = f_x + self.dist_x * math.cos((rad + 1.5 * pi)%(2 * pi)), f_y + self.dist_x * math.sin((rad + 1.5 * pi)%(2 * pi))
                    x2, y2 = s_x + self.dist_x * math.cos((rad + 1.5 * pi)%(2 * pi)), s_y + self.dist_x * math.sin((rad + 1.5 * pi)%(2 * pi))
                    ...
                    s_x, s_y = f_x + self.dist_diag * math.cos(rad), f_y + self.dist_diag * math.sin(rad)
                    x1, y1 = f_x + self.dist_y * math.cos(rad + self.ang_a), f_y + self.dist_y * math.sin(rad + self.ang_a)
                    x4, y4 = f_x + self.dist_x * math.cos(rad - self.ang_b), f_y + self.dist_x * math.sin(rad - self.ang_b)
                    ...
                    s_x, s_y = f_x + self.dist_x * math.cos(rad), f_y + self.dist_x * math.sin(rad)
                    x1, y1 = f_x + self.dist_y * math.cos((rad + 0.5 * pi)%(2 * pi)), f_y + self.dist_y * math.sin((rad + 0.5 * pi)%(2 * pi))
                    x2, y2 = s_x + self.dist_y * math.cos((rad + 0.5 * pi)%(2 * pi)), s_y + self.dist_y * math.sin((rad + 0.5 * pi)%(2 * pi))
                ...
                if s_node[0] == '11': # First Node = Tag4, Second Node = Tag1
                    s_x, s_y = f_x + self.dist_diag * math.cos(rad), f_y + self.dist_diag * math.sin(rad)
                    x3, y3 = f_x + self.dist_x * math.cos(rad + self.ang_b), f_y + self.dist_x * math.sin(rad + self.ang_b)
                    x2, y2 = f_x + self.dist_y * math.cos(rad - self.ang_a), f_y + self.dist_y * math.sin(rad - self.ang_a)
                    ...
                    s_x, s_y = f_x + self.dist_y * math.cos(rad), f_y + self.dist_y * math.sin(rad)
                    x3, y3 = f_x + self.dist_x * math.cos((rad + 0.5 * pi)%(2 * pi)), f_y + self.dist_x * math.sin((rad + 0.5 * pi)%(2 * pi))
                    x1, y1 = s_x + self.dist_x * math.cos((rad + 0.5 * pi)%(2 * pi)), s_y + self.dist_x * math.sin((rad + 0.5 * pi)%(2 * pi))
                    ...
                    s_x, s_y = f_x + self.dist_x * math.cos(rad), f_y + self.dist_x * math.sin(rad)
                    x2, y2 = f_x + self.dist_y * math.cos((rad + 1.5 * pi)%(2 * pi)), f_y + self.dist_y * math.sin((rad + 1.5 * pi)%(2 * pi))
                    x1, y1 = s_x + self.dist_y * math.cos((rad + 1.5 * pi)%(2 * pi)), s_y + self.dist_y * math.sin((rad + 1.5 * pi)%(2 * pi))
            ...
            target = [round((self.m * mid_34[0] + self.n * mid_12[0])/(self.m + self.n),3), round((self.m * mid_34[1] + self.n * mid_12[1])/(self.m + self.n),3)]
